...
#define kTimeInterval 6.0
#define kAnimateDuration 0.5
#define kBottomMargin 0


@interface ImageLoopView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) Top_StoriesView *leftImageView;
@property (nonatomic, strong) Top_StoriesView *middleImageView;  // 中间图片会始终在UIScrollView的中间位置
@property (nonatomic, strong) Top_StoriesView *rightImageView;
...
- (void)setTop_stories:(NSArray<Top_Stories *> *)top_stories {
    _top_stories = top_stories;
    if (_top_stories == nil) {
        return;
    }
    [self setImages:top_stories];
...
+ (instancetype)imageLoopView {
    return [[ImageLoopView alloc] init];
}
...
- (void) setupUI {
    ...
    // 2. Left Middle Right
    _leftImageView = [[Top_StoriesView alloc] init];
    [scrollView addSubview:_leftImageView];
    
    _middleImageView = [[Top_StoriesView alloc] init];
    [scrollView addSubview:_middleImageView];
    
    _rightImageView = [[Top_StoriesView alloc] init];
...
- (void)layoutSubviews {
    ...
    // UIPageControl
    CGFloat pageConntrolWidth = 35 * self.images.count;
    CGFloat x = 0;
...
//        [_rightImageView setImageWithURL:[NSURL URLWithString:((Top_Stories *)_images[0]).image]];
        [_rightImageView setTop_story:(Top_Stories *)_images[0]];
    } else {
//        [_rightImageView setImageWithURL:[NSURL URLWithString:((Top_Stories *)_images[_currentIndex + 1]).image]];
        [_rightImageView setTop_story:(Top_Stories *)_images[_currentIndex + 1]];
    }
}
